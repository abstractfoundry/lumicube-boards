...
static void processErrorStatus(void)
{
    ...
    // 000: No Error
    // 001: Stuff Error - rx - invalid data
    // 010: Form Error - rx - invalid data
    // 011: Acknowledgment Error - tx - Nothing ack'd the tx
    // 100: Bit recessive Error - tx bit was not detected correctly by rx - probably something holding the bus
    // 101: Bit dominant Error - tx bit was not detected correctly by rx - probably something holding the bus
...
//   if (BXCAN->TSR & (CANARD_STM32_CAN_TSR_TERR0 | CANARD_STM32_CAN_TSR_TERR1 | CANARD_STM32_CAN_TSR_TERR2)) {
//       g_stats.error_count++;
//   }
//   if (BXCAN->ESR & CANARD_STM32_CAN_ESR_TEC_MASK) {
//       g_stats.error_count++;
//   }
...
bool txRequestCompleted() {
    if (BXCAN->TSR & (CANARD_STM32_CAN_TSR_RQCP0 | CANARD_STM32_CAN_TSR_RQCP1 | CANARD_STM32_CAN_TSR_RQCP2)) {
        return 1;
    }
    return 0;
}
